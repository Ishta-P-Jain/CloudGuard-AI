...
class OpenRdpRule(BaseRule):
    rule_id = "EC2_OPEN_RDP"
    service = "EC2"
    ...
    def evaluate(self, resource: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for index, rule in enumerate(_extract_ingress_rules(resource)):
            ...
            if _ports_cover_rdp(from_port, to_port, protocol) and _has_public_cidr(cidrs):
                findings.append(
                    self.build_finding(
                        resource_id=group_id,
                        description="This security group allows RDP from the internet.",
                        evidence={
                            "group_id": group_id,
...
def _ports_cover_rdp(from_port: int, to_port: int, protocol: str) -> bool:
    if protocol in {"-1", "all"}:
        return True
    return from_port <= 3389 <= to_port
